        ...
        public static WorldMeshData GenerateWorldMesh(World world)
        {
            ...
            void AddWorldTileToVoxelMeshData(int x, int y)
            {
                ...
                mainMeshData.AddVoxelSurface(
                    ...
                    DatabaseManager.Instance.worldTileDatabaseAssets[tile.worldTileID].TextureInfo.Up,
                    10, EVoxelSurface.up);
                ...
                void AddFullSufaceToVoxelMeshData(WorldTile neighbourTile, EVoxelSurface surface)
                {
                    ...
                    {
                        mainMeshData.AddVoxelSurface(new Vector3(x, tile.altitude, y), DatabaseManager.Instance.worldTileDatabaseAssets[tile.worldTileID].TextureInfo.Side, 10, surface);
                        for (int i = 1; i < tile.altitude; i++)
                        {
                            mainMeshData.AddVoxelSurface(new Vector3(x, tile.altitude - i, y), DatabaseManager.Instance.worldTileDatabaseAssets[tile.worldTileID].TextureInfo.RepeatingSide, 10, surface);
                        }
                    }
