...
namespace MyProject.VoxelEngine
{
    ...
    {
        public static VoxelSpaceMeshData GenerateMesh(VoxelSpace world)
        {
            var spaceMeshData = new VoxelSpaceMeshData();

            world.IterateAllCoordinates(HandleMeshDataAt);
            return spaceMeshData;

            // 挨个加进MeshData
            void HandleMeshDataAt(int x, int y)
            {
                var voxel = world.GetVoxelAt(x, y);
                var database = DatabaseManager.Instance.Voxels.Assets[voxel.voxelID];
                if (database.RenderCategory == ERenderCategory.opaque) AddRenderMesh(spaceMeshData.opaque);
                if (database.HasCollider) AddColliderMesh(spaceMeshData.collider);

                void AddRenderMesh(VoxelMeshData meshData)
                {
                    // 处理水平表面。必定显示，所以直接加入
                ...
                void AddColliderMesh(VoxelMeshData meshData)
                {
                    meshData.AddVoxelSurface(new Vector3(x, voxel.altitude, y), EVoxelSurface.up);
                    ...
                    void AddFullSufaceToVoxelMeshData(Voxel neighbourVoxel, EVoxelSurface surface)
                    {
                        ...
                        if (neighbourVoxel == null)  // 没邻居就一路刷到底
                        {
                            meshData.AddVoxelSurface(new Vector3(x, voxel.altitude, y), surface);
                            for (int i = 1; i < voxel.altitude; i++)
                            {
                                meshData.AddVoxelSurface(new Vector3(x, voxel.altitude - i, y), surface);
                            }
                        }
                        else if (neighbourVoxel.altitude < voxel.altitude) // 只有当邻居比我矮一个头时，才继续刷下去
                        {
